...
(function () {
  ...
  function normalizaEstado() {
    var base = window.SEED.parametros;
    db.params = db.params || {};
    ["meses", "categorias", "statuses", "servicos", "tiposTransacao", "palavrasAeroporto", "dePara"].forEach(function (k) {
      if (!db.params[k]) db.params[k] = clone(base[k]);
    });
    db.params.regras = Object.assign({}, base.regras, db.params.regras || {});
    db.uber = db.uber || [];
  ...
  function restaurarBase() { substituirEstado(estadoInicial()); }
  ...
  function salvarViagem(dados) {
    var alvo = dados.id ? viagemPorId(dados.id) : null;
    if (!alvo) {
      alvo = viagemVazia();
      alvo.id = proximoId();
    ...
    CAMPOS_VIAGEM.forEach(function (k) {
      if (dados[k] !== undefined) alvo[k] = dados[k];
    });
    salvar();
    return alvo;
  }
  ...
  function adicionarCorridas(linhas) {
    ...
    if (novas) salvar();
    return { novas: novas, repetidas: repetidas };
  }
  ...
  function limparUber() { db.uber = []; salvar(); }
