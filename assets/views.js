...
(function () {
  ...
  /**
   * @param {object} o
   * @param {string[]} o.rotulos      rótulo de cada coluna
   * @param {string[]} [o.chaves]     chave de cada coluna; presente, a coluna vira clicável
   * @param {object[]} o.series       [{nome, cor, valores: []}]
   * @param {object} [o.referencia]   {valor, rotulo} — linha de alvo
   */
  function barrasEmpilhadas(o) {
    var rotulos = o.rotulos, series = o.series, alturaBarra = o.altura, referencia = o.referencia;
    var chaves = o.chaves || [];
    ...
    // Variação contra o mês anterior, escrita em cima da pilha. Só a partir
    // do segundo mês com movimento — o primeiro não tem com o que comparar.
    var anterior = null;
    rotulos.forEach(function (rot, i) {
      ...
      // Cada mês é um grupo: o clique pega a coluna inteira, não só a fatia
      // debaixo do cursor, e a área sensível vai do eixo ao topo do gráfico.
      var clicavel = total > 0 && chaves[i];
      svg += clicavel
        ? '<g class="coluna" data-acao="ver-mes" data-mes="' + esc(chaves[i]) + '" role="button" tabindex="0" ' +
          'aria-label="Abrir ' + esc(rot) + '">' +
          '<rect class="area-clique" x="' + (cx - largura / 2 + 2) + '" y="' + T +
          '" width="' + (largura - 4) + '" height="' + plotH + '" rx="4"/>'
        : "<g>";

      if (total > 0 && anterior) {
        var pct = (total - anterior) / anterior * 100;
        var sobe = pct > 0;
        var topoBarra = T + plotH - (total / topo) * plotH;
        svg += '<text class="delta ' + (Math.abs(pct) < 0.5 ? "" : sobe ? "delta-sobe" : "delta-cai") +
          '" x="' + cx + '" y="' + (topoBarra - 6) + '" text-anchor="middle" font-size="10.5" font-weight="650">' +
          (Math.abs(pct) < 0.5 ? "=" : (sobe ? "▲ +" : "▼ ") + C.brl(pct, 0) + "%") + "</text>";
      }
      if (total > 0) anterior = total;
  ...
  function barrasHorizontais(itens) {
    ...
    return '<div style="display:flex;flex-direction:column;gap:9px">' + itens.map(function (i) {
      ...
      return '<div style="display:grid;grid-template-columns:minmax(0,1fr) auto;gap:4px 12px;align-items:center">' +
        ...
        '<span class="num" style="font-size:12.5px">' +
        (i.extra ? '<span class="t-sub" title="Custo médio por viagem, sem Uber" style="margin-right:12px">' + esc(i.extra) + "</span>" : "") +
        C.moeda(i.valor) + "</span>" +
  ...
  function orcamentoCard(o, ano) {
    ...
    return '<div class="card" style="grid-column:1/-1"><div class="card-head">' +
      ...
      (o.nMeses === 1 ? " mês com lançamento" : " meses com lançamento") +
      " · média de " + C.moeda(o.mediaMensal) + "/mês</span></div>" +
      ...
      (estourou
        ? "<strong>" + o.nEstourados + " de " + o.nMeses + " meses fecharam acima do teto.</strong> " +
          ...
          " Mantido o ritmo, o ano fecha em " + C.moeda(o.mediaMensal * 12) + " ante " +
          C.moeda(o.budget * 12) + " de orçamento anual."
        : "<strong>Dentro do orçamento.</strong> Sobra de " + C.moeda(o.saldo) +
          " no acumulado do ano; mantido o ritmo, fecha em " + C.moeda(o.mediaMensal * 12) + ".") +
      "</div>" +
  ...
  function analisesCard(r, o, pessoas, uber, ano) {
    ...
    // O confronto com o orçamento fica todo no cartão de cima; aqui só entra o
    // que ele não responde: para onde o dinheiro vai e a que preço.
    ...
    // 7. Quanto custa a diária de hotel, já sem o aluguel de apartamento
    var hotel = { valor: 0, noites: 0 };
    C.viagens().forEach(function (v) {
      if (C.anoDe(v.mesRef) !== String(ano) || v.locacao || v.cancelada) return;
      hotel.valor += v.hospedagem;
      hotel.noites += v.noitesEfetivas;
    });
    if (hotel.noites && hotel.valor) {
      itens.push({
        rotulo: "Diária média de hotel",
        valor: C.moeda(hotel.valor / hotel.noites),
        nota: C.moeda(hotel.valor) + " de hotel em " + hotel.noites + " pernoites, já fora o aluguel " +
              "de apartamento. É este o número a levar para uma negociação de tarifa corporativa.",
        tom: ""
      });
    }
  ...
  /**
   * Detalhe de um mês: todas as linhas de custo, o confronto com o teto e os
   * lançamentos que mais pesaram. É o que abre ao clicar na barra do mês.
   */
  function detalheMes(mesRef, ano) {
    var r = C.resumoAnual(ano);
    var o = C.orcamento(ano);
    var i = r.meses.indexOf(mesRef);
    var anterior = i > 0 ? r.meses[i - 1] : null;
    var linhaOrc = o.meses.filter(function (l) { return l.mes === mesRef; })[0];
    var total = r.totalMes[mesRef] || 0;

    var html = "";

    if (linhaOrc) {
      var estourou = linhaOrc.saldo < 0;
      html += '<div class="mes-topo">' +
        '<div><span class="eyebrow">Realizado</span><div class="orc-numero ' +
        (estourou ? "acima" : "abaixo") + '">' + C.moeda(total) + "</div></div>" +
        '<div><span class="eyebrow">Orçamento</span><div class="orc-numero">' +
        C.moeda(linhaOrc.budget) + "</div></div>" +
        '<div><span class="eyebrow">' + (estourou ? "Estouro" : "Saldo") + "</span>" +
        '<div class="orc-numero ' + (estourou ? "acima" : "abaixo") + '">' +
        (estourou ? "−" : "+") + C.moeda(Math.abs(linhaOrc.saldo)) + "</div>" +
        '<span class="t-sub">' + C.brl(linhaOrc.consumo * 100, 0) + "% do teto</span></div>" +
        "</div>" + medidor(total, linhaOrc.budget);
    } else {
      html += '<div class="mes-topo"><div><span class="eyebrow">Realizado</span>' +
        '<div class="orc-numero">' + C.moeda(total) + "</div></div></div>";
    }

    // Todas as linhas de custo do mês, com o que variou contra o mês anterior.
    html += '<div class="table-wrap" style="margin-top:14px"><table><thead><tr>' +
      "<th>Linha de custo</th><th class=\"n\">Valor</th><th class=\"n\">% do mês</th>" +
      '<th class="n">vs ' + (anterior ? esc(C.mesRotulo(anterior)) : "M-1") + "</th></tr></thead><tbody>" +
      r.categorias.map(function (cat, idx) {
        var v = r.porCategoria[cat][mesRef] || 0;
        var ant = anterior ? (r.porCategoria[cat][anterior] || 0) : 0;
        return { cat: cat, cor: CORES[idx % CORES.length], v: v, ant: ant };
      }).filter(function (l) { return l.v > 0 || l.ant > 0; })
        .sort(function (a, b) { return b.v - a.v; })
        .map(function (l) {
          var sub = l.cat === "Hospedagem" && r.locacaoMes[mesRef]
            ? '<tr class="sublinha"><td>↳ dos quais locação de apartamento</td>' +
              '<td class="n">' + C.moeda(r.locacaoMes[mesRef]) + "</td><td></td><td></td></tr>"
            : "";
          return "<tr><td><span class='legend-swatch' style='display:inline-block;background:" + l.cor +
            ";margin-right:7px'></span>" + esc(l.cat) + "</td>" +
            '<td class="n">' + (l.v ? C.moeda(l.v) : "—") + "</td>" +
            '<td class="n t-sub">' + (total ? C.brl(l.v / total * 100, 0) + "%" : "—") + "</td>" +
            '<td class="n">' + (anterior ? variacao(l.v, l.ant) : "—") + "</td></tr>" + sub;
        }).join("") +
      "</tbody><tfoot><tr><td><strong>TOTAL DO MÊS</strong></td>" +
      '<td class="n"><strong>' + C.moeda(total) + "</strong></td>" +
      '<td class="n"><strong>100%</strong></td>' +
      '<td class="n">' + (anterior ? variacao(total, r.totalMes[anterior] || 0) : "—") + "</td>" +
      "</tr></tfoot></table></div>";

    // O que puxou o mês
    var doMes = C.viagens().filter(function (v) { return v.mesRef === mesRef; })
      .sort(function (a, b) { return b.total - a.total; });
    if (doMes.length) {
      html += '<div class="section-label" style="margin-top:16px"><span class="eyebrow">' +
        "O que puxou o mês</span></div>" +
        '<div class="table-wrap" style="max-height:260px"><table class="compacta"><thead><tr>' +
        "<th>Colaborador</th><th>Período</th><th>Área</th><th class=\"n\">Total</th>" +
        "</tr></thead><tbody>" +
        doMes.slice(0, 8).map(function (v) {
          return "<tr><td>" + esc(v.colaborador) +
            (v.locacao ? ' <span class="chip">apartamento</span>' : "") + "</td>" +
            '<td class="t-sub num">' + C.fmtDataCurta(v.dataIda) + " → " + C.fmtDataCurta(v.dataVolta) + "</td>" +
            "<td>" + esc(v.area) + "</td>" +
            '<td class="n">' + C.moeda(v.total) + "</td></tr>";
        }).join("") +
        "</tbody></table></div>" +
        '<p class="t-sub" style="margin:8px 0 0">' + doMes.length +
        (doMes.length === 1 ? " lançamento" : " lançamentos") + " no mês · " +
        Object.keys(r.pessoasMes[mesRef] || {}).length + " pessoas viajaram" +
        (r.totalUber ? " · Uber do mês: " + C.moeda(r.porCategoria["Uber Corporativo"][mesRef] || 0) : "") +
        "</p>";
    }
  ...
  function painel(estado) {
    ...
    html += '<div class="card">' +
      '<div class="card-head"><h2>Custo mensal por categoria</h2>' +
      '<span class="t-sub">clique no mês para abrir o detalhe</span>' +
      '<span class="grow"></span><span class="eyebrow">' + esc(ano) + "</span></div>" +
      '<div class="card-body">' +
      barrasEmpilhadas({
        rotulos: r.meses.map(C.mesRotulo),
        chaves: r.meses,
        series: series,
        referencia: o.budget ? { valor: o.budget, rotulo: "Orçamento R$ " + C.brlCurto(o.budget) + "/mês" } : null
      }) +
    ...
    html += '<div class="card"><div class="card-head"><h2>Custo por área</h2>' +
      '<span class="t-sub">clique no título da coluna para ordenar</span></div>' +
      '<div class="card-body flush"><div class="table-wrap"><table><thead><tr>' +
      thOrdenavel("Área", "area", false, ordemArea, "areaOrdem") +
      mesesAtivos.map(function (m) { return thOrdenavel(C.mesRotulo(m), m, true, ordemArea, "areaOrdem"); }).join("") +
      thOrdenavel("Total", "total", true, ordemArea, "areaOrdem") + "</tr></thead><tbody>" +
    ...
    html += '<div class="card"><div class="card-head"><h2>Top 10 colaboradores</h2>' +
      ...
      '<div class="card-body">' + barrasHorizontais(pessoas.slice(0, 10).map(function (p) {
        var media = p.viagens ? (p.total - p.uber) / p.viagens : 0;
        return { nome: p.nome, sub: p.viagens + (p.viagens === 1 ? " viagem" : " viagens"),
  ...
  function uberView(estado) {
    ...
    html += '<div class="card"><div class="card-head"><h2>Evolução mensal</h2></div>' +
      ...
      '<th class="n">Indevidas</th><th class="n">vs M-1</th>' +
      "</tr></thead><tbody>" +
      mesesU.map(function (m, i) {
        ...
        return "<tr><td>" + esc(C.mesRotulo(m)) + '</td><td class="n">' + d.n + "</td>" +
          ...
          '<td class="n' + (d.indevidas ? " indevido" : " zero") + '" title="' +
            (d.nIndevidas ? esc(d.nIndevidas + " corrida" + (d.nIndevidas > 1 ? "s" : "") + " marcada" + (d.nIndevidas > 1 ? "s" : "") + " como não devida" + (d.nIndevidas > 1 ? "s" : "")) : "") +
            '">' + (d.indevidas ? C.moeda(d.indevidas) : "—") + "</td>" +
    ...
    html += '<div class="card"><div class="card-head"><h2>Por cidade</h2>' +
      '<span class="grow"></span><span class="t-sub">10 maiores</span></div>' +
    ...
    html += '<div class="card"><div class="card-head"><h2>Por funcionário</h2>' +
      '<span class="grow"></span><span class="t-sub">10 maiores de ' + esc(u.porPessoa.length) + "</span></div>" +
      '<div class="card-body">' + barrasHorizontais(u.porPessoa.slice(0, 10).map(function (c) {
        return { nome: c.nome, sub: c.n + (c.n === 1 ? " corrida" : " corridas"), valor: c.valor };
      })) + "</div></div>";
    ...
    html += '<div class="card"><div class="card-head"><h2>Uso do Uber por área</h2>' +
      ...
      '<th class="n">Ticket</th><th class="n">% do total</th>' +
    ...
    // ---- pontos de atenção: o que está em aberto e o que já foi julgado indevido ----
    ...
    html += '<div class="card"><div class="card-head"><h2>Pontos de atenção</h2></div>' +
      ...
      '<section class="bloco"><div class="bloco-head"><h3>Em aberto, por motivo</h3>' +
      '<span class="grow"></span><span class="t-sub">o que foi validado como devido sai da conta</span></div>' +
      ...
      '<section class="bloco"><div class="bloco-head"><h3>Corridas indevidas</h3>' +
      ...
      "</div>" +
      (u.indevidas.length
        ? '<div class="lista-indevidas">' + u.indevidas.map(function (c) {
            return "<details><summary>" +
              '<span class="ind-nome">' + esc(c.colaborador || c.nomeRelatorio || "—") +
              '<span class="t-sub"> · ' + esc(c.area || "") + "</span></span>" +
              '<span class="num ind-valor">' + C.moeda(c.valor) + "</span></summary>" +
              '<div class="ind-detalhe">' +
              '<div><span class="eyebrow">Data</span> <span class="num">' +
              (c.data ? esc(C.fmtData(c.data)) : "—") + (c.hora ? " · " + esc(c.hora) : "") + "</span></div>" +
              '<div><span class="eyebrow">Trajeto</span> ' + esc(c.origem || "—") + " → " + esc(c.destino || "—") + "</div>" +
              '<div><span class="eyebrow">Justificativa</span> ' + esc(c.conferencia.justificativa || "—") + "</div>" +
              (c.conferencia.comentario ? '<div><span class="eyebrow">Comentário</span> ' + esc(c.conferencia.comentario) + "</div>" : "") +
              '<div class="t-sub">Validado por ' + esc(c.conferencia.por || "—") + " em " +
              esc(C.fmtData((c.conferencia.em || "").slice(0, 10))) + " · " +
              '<button class="link-inline" data-acao="ver-corrida" data-id="' + esc(c.chave) + '">ver a corrida completa</button></div>' +
              "</div></details>";
          }).join("") +
          '<div class="ind-total"><span>TOTAL</span><span class="num">' + C.moeda(u.valorNaoDevidas) + "</span></div></div>"
        : '<div style="padding:16px">' + vazio("Nenhuma corrida marcada como indevida",
